...
class AcceptedApplicationDetailPage extends ConsumerStatefulWidget {
  AcceptedApplicationDetailPage({super.key});
  @override
  ConsumerState<AcceptedApplicationDetailPage> createState() =>
      _AcceptedApplicationDetailPageState();
}

ApplicationService _applicationService = ApplicationService();

class _AcceptedApplicationDetailPageState
    extends ConsumerState<AcceptedApplicationDetailPage> {
  @override
  Widget build(BuildContext context) {
    final applicationData = ref.watch(acceptedApplicationProvider);
    return applicationData.when(
      data: (data) {
        return ListView.builder(
          itemCount: data.length,
          itemBuilder: (context, index) {
            final applicationData = data[index];
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            LabelValueWidget(
                                label: 'Skills',
                                value: applicationData.volunteerSkills!
                                    .join(', ')),
                            LabelValueWidget(
                                label: 'Interest',
                                value: applicationData.volunteerInterests!
                                    .join(', ')),
                            const SizedBox(height: 8.0),
                            SizedBox(height: 20.h),
                            Row(
